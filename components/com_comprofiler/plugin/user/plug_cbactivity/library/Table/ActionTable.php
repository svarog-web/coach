...
class ActionTable extends OrderedTable
{
	...
	public function icon()
	{
		...
		$icon			=	null;
		$actionClass	=	'streamIconAction' . ucfirst( strtolower( preg_replace( '/[^-a-zA-Z0-9_]/', '', $this->get( 'value' ) ) ) );

		if ( $this->get( 'icon' ) ) {
			$icon		=	'<img src="' . $_CB_framework->getCfg( 'live_site' ) . '/images/' . htmlspecialchars( $this->get( 'icon' ) ) . '" class="streamIconAction ' . $actionClass . ' img-responsive-inline" />';
		} elseif ( $this->get( 'class' ) ) {
			$icon		=	'<span class="streamIconAction ' . $actionClass . ' ' . htmlspecialchars( $this->get( 'class' ) ) . '"></span>';
		}
