...
class WordPressProvider extends Provider
{
	...
	/** @var string  */
	protected $fields			=	'ID,display_name,username,email,avatar_URL';
	/** @var array  */
	...
	public function authenticate()
	{
		...
		if ( ( ! $this->session()->get( 'wordpress.state' ) ) || ( $this->session()->get( 'wordpress.state' ) != Application::Input()->get( 'state', null, GetterInterface::STRING ) ) ) {
			$code				=	null;
		}
	...
	{
		return ( $this->session()->get( 'wordpress.access_token' ) != '' );
	}
	...
	public function profile( $fields = null )
	{
		...
		if ( $response instanceof Registry ) {
			...
			if ( $profile->get( 'id' ) ) {
				$this->session()->set( 'wordpress.id', $profile->get( 'id' ) );
			}
	...
	public function api( $api, $type = 'GET', $params = array(), $headers = array() )
	{
		...
		if ( $this->session()->get( 'wordpress.access_token' ) ) {
			$headers['Authorization']	=	'Bearer ' . $this->session()->get( 'wordpress.access_token' );
		}
