...
class HTML_cbantispamAttempts
{
	...
	/**
	 * @param cbantispamAttemptsTable[] $rows
	...
	static public function showAttempts( $rows, $pageNav, $viewer, $user, $tab, $plugin )
	{
		...
		$return						=	'<div class="attemptsTab">'
									.		'<form action="' . $_CB_framework->userProfileUrl( (int) $user->get( 'id' ), true, (int) $tab->get( 'tabid' ) ) . '" method="post" name="attemptsForm" id="attemptsForm" class="attemptsForm">'
									.			'<table class="attemptsContainer table table-hover table-responsive">'
									.				'<thead>'
									.					'<tr>'
		...
		if ( $rows ) foreach ( $rows as $row ) {
			$return					.=					'<tr>'
									...
									.						'<td style="width: 20%;" class="text-center">' . $row->get( 'type' ) . '</td>'
									.						'<td style="width: 10%;" class="text-center hidden-xs">' . $row->get( 'count' ) . '</td>'
									.						'<td style="width: 1%;" class="text-right">'
									.							'<a href="javascript: void(0);" onclick="if ( confirm( \'' . addslashes( CBTxt::T( 'Are you sure you want to delete this Attempt?' ) ) . '\' ) ) { location.href = \'' . $_CB_framework->pluginClassUrl( $plugin->element, false, array( 'action' => 'attempt', 'func' => 'delete', 'id' => (int) $row->get( 'id' ), 'usr' => (int) $user->get( 'id' ), 'tab' => (int) $tab->get( 'tabid' ) ) ) . '\'; }" title="' . htmlspecialchars( CBTxt::T( 'Delete' ) ) . '"><span class="fa fa-trash-o"></span></a>'
									.						'</td>'
									.					'</tr>';
		} else {
			$return					.=					'<tr>'
