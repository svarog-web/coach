			...
			case 'htmledit':
				if ( $field->name == 'username' ) {
					...
					if ( ! Application::Cms()->getClientId() ) {
						$return					.=	'<div class="cbCurrentProfileURL small text-muted">' . CBTxt::Th( 'YOUR_CURRENT_PROFILE_URL_IS_URL', 'Your current Profile URL is: [url]', array( '[url]' => $_CB_framework->viewUrl( 'userprofile', true, array( 'user' => $user->get( 'id', 0, GetterInterface::INT ) ) ) ) ) . '</div>';
					}
			...
			||	( ( isset( $ueConfig['frontend_userparams'] ) ) ? ( $ueConfig['frontend_userparams'] == 1 ) : in_array( $_CB_framework->getCfg( "frontend_userparams" ), array( '1', null) ) ) )
		{
			...
			if ( checkJversion( '3.2+' ) && $user->id ) {
				...
				if ( count( $twoFactorMethods ) > 1 ) {
					...
					$otpConfig					=	$model->getOtpConfig( $user->id );
					$twoFactorMethod			=	cbGetParam( $postdata, 'jform[twofactor][method]', 'none' );
